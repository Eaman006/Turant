...
export default function RateDriverModal({
  ...
  useEffect(() => {
    ...
    const fetchExistingRating = async () => {
      ...
      try {
        const res = await fetch(`/api/rate-cab?cabId=${cabId}&userId=${currentUser.uid}`);
        if (res.ok) {
  ...
  const submitRating = async () => {
    ...
    try {
      const res = await fetch("/api/rate-cab", {
        method: "POST",
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm rounded-3xl bg-white shadow-xl border border-gray-100 p-6 font-[family-name:var(--font-poppins)] text-center">
        <h3 className="text-[#201B10] font-extrabold text-xl mb-2">Rate Driver</h3>
